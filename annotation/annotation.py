...
#        Significantly modified to add more image types, image folders, format, labelling saves, format selection, and corresponding sequences of images. 
#        Currently supports Kitti and YOLO(darknet) output formats
# For Python 2.7 
# install pillow by pip or anaconda python
# install image by pip or anaconda python
# For Python 3
# Python 3 + Pillow on Ubuntu, do the below
# sudo apt-get install python-imaging-tk
# sudo apt-get install python3-pil.imagetk
# Or you can install all the important requirements by installing anaconda python
...
class Annotation():
    ...
    def loadDir(self, dbg = False):
        ...
        for files in imageFileTypes:
            ...
            self.imageList.sort(key=lambda f: int(''.join(filter(str.isdigit,f))))
